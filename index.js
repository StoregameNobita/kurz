...
const starts = async (kurr = new WAConnection()) => {
	...
	console.log(color(figlet.textSync('Nobita-Botz', {
		font: 'Standard',
...
console.log(color('> HARI ','silver'), color(`${tampilHari}\n`,'mediumseagreen'))
	kurr.browserDescription = [ 'Nobita-Botz', 'ubuntu', '3.0' ]
...
kurr.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
   ...
   // owner
    ...
    const htod = "6285749037935@s.whatsapp.net"
	...
	try {
		...
		if (anu.action == 'add') {
			...
			try {
				...
				sendButImage(anu.jid, teks, `Nobita-Botz`, img,but = [{buttonId:`donasi`, 
			...
			try {
				...
				teks = `Goodbye @${num.split('@')[0]}\nSelamat Tinggal Di Group,Semoga Tenang Di Alam Sana: ${mdata.subject}`

				sendButImage(anu.jid, teks, `Nobita-Botz`, img,but = [{buttonId: `Hello World!`, buttonText: {displayText: `SAYONARA !!`}, type: 1}], options = {contextInfo: {mentionedJid: [num, htod]}})
...
kurr.on('group-update', async (anu) => {
    ...
    kurr.on('chat-update', async (message) => {
        ...
        ownerNumber = ["6285749037935@s.whatsapp.net","6285749037935@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
        dtod = "6285749037935@s.whatsapp.net"
